function delete_register()
{
  ...
  if echo "$answer" | grep -iq "^y" ; then
      ...
      for i in `seq 1 20`; do
          echo "curlzzz -X DELETE -u openregister:$password https://$register.$phase.openregister.org/delete-register-data"
...
function load_rsf()
{
  ...
  if echo "$answer" | grep -iq "^y" ; then
    ...
    echo "cat $OPENREGISTER_BASE/tmp.rsf | curlzzz -x POST -u openregister:$password --data-binary @- https://$register.$phase.openregister.org/load-rsf --header 'content-type:application/uk-gov-rsf'"
  fi
}
